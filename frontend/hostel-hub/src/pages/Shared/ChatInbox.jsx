...
const getParticipantName = (participant) =>
  participant?.hostelName || participant?.name || "Unknown user";
...
const getThreadPreview = (thread) =>
  thread?.lastMessage || `Start chatting about ${thread?.listing?.title || "this listing"}`;
...
const ChatInbox = () => {
  ...
  const filteredThreads = useMemo(() => {
    ...
    return threads.filter((thread) => {
      const participantName = getParticipantName(getThreadParticipant(thread));
      const haystack = [
        participantName,
        ...
        <div className="space-y-3 px-3 py-4">
          {Array.from({ length: 6 }).map((_, index) => (
            <div
              ...
              className="flex items-center gap-3 rounded-3xl bg-white px-4 py-3 shadow-sm ring-1 ring-slate-100"
            >
    ...
    if (filteredThreads.length === 0) {
      return (
        <div className="px-6 py-10 text-center">
          <div className="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-white text-slate-400 shadow-sm ring-1 ring-slate-200">
          ...
          <p className="mt-2 text-sm text-slate-500">
            {threads.length === 0
              ? user?.role === "owner"
                ? "Renter messages will show up here once someone starts chatting."
                : "Start chatting from a hostel listing and the conversation will appear here."
              : "Try another name, listing, or message keyword."}
          </p>
        </div>
      );
    }
          ...
          <div className="flex items-center gap-3">
            <div className="relative shrink-0">
              {participantImage ? (
                ...
                <div className="flex h-14 w-14 items-center justify-center rounded-full bg-slate-200 text-sm font-semibold text-slate-700">
                  {getInitials(participantName)}
                </div>
              )}
              {thread.unreadCount > 0 ? (
                <span className="absolute -bottom-0.5 -right-0.5 h-4 w-4 rounded-full border-2 border-[#f8fafd] bg-blue-600" />
              ) : null}
            </div>
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="mt-0.5 truncate text-xs text-slate-500">
                {thread?.listing?.title || "Hostel listing"}
              </p>

              <div className="mt-1 flex items-center gap-2">
                <p
                  className={`truncate text-sm ${
                    thread.unreadCount > 0 ? "font-medium text-slate-700" : "text-slate-500"
                  }`}
                >
                  {getThreadPreview(thread)}
                </p>
  ...
  const renderComposer = (disabled = false) => (
    <div className="sticky bottom-0 z-20 border-t border-slate-200 bg-white/95 px-4 py-4 backdrop-blur sm:px-6">
      <div className="flex w-full items-end gap-3">
        <div className="flex min-h-14 flex-1 items-end rounded-[30px] border border-slate-200 bg-slate-50 px-4 py-2 shadow-inner">
          <textarea
            value={draftMessage}
            ...
            placeholder={
              disabled
                ? "Choose a listing or conversation to start chatting."
                : "Aa"
            }
            className="max-h-32 min-h-8 w-full resize-none bg-transparent py-2 text-sm text-slate-900 outline-none placeholder:text-slate-400 disabled:text-slate-400"
        ...
        <button
          ...
          className="inline-flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-blue-600 text-white shadow-lg shadow-blue-200 transition-all duration-200 hover:bg-blue-700 disabled:bg-slate-200 disabled:text-slate-500 disabled:shadow-none"
        >
          ...
          <div className="flex min-w-0 items-center gap-3">
            {showBackButton ? (
              <button
                ...
                className="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-full text-slate-600 hover:bg-slate-100 lg:hidden"
            ...
            <div className="min-w-0">
              ...
              <p className="truncate text-sm text-slate-500">
                {user?.role === "owner" ? "Renter" : "Hostel owner"}
  ...
  const renderChatPanel = () => {
    if (loadingChat || loadingListing) {
      return (
        <div className="flex h-full min-h-0 w-full flex-col overflow-hidden bg-[#f4f7fb]">
          ...
          <div className="relative min-h-0 flex-1 overflow-hidden bg-[radial-gradient(circle_at_top,_rgba(59,130,246,0.08),_transparent_30%),linear-gradient(180deg,_#f6f9fd_0%,_#eff4fb_100%)] px-4 py-6 sm:px-6">
    ...
    if (currentThread) {
      ...
      return (
        <div className="relative flex h-full min-h-0 w-full flex-col overflow-hidden bg-[#f4f7fb]">
          {renderThreadHeader({
            participant,
          ...
          <div
            ...
            className="min-h-0 flex-1 overflow-y-auto bg-[radial-gradient(circle_at_top,_rgba(59,130,246,0.08),_transparent_30%),linear-gradient(180deg,_#f6f9fd_0%,_#eff4fb_100%)] px-4 py-6 sm:px-6"
          >
            <div className="flex w-full flex-col">
              <div className="mb-8 flex flex-col items-center text-center">
                {participantImage ? (
                  ...
                  <div className="flex h-20 w-20 items-center justify-center rounded-full bg-slate-200 text-lg font-semibold text-slate-700 shadow-sm">
                    {getInitials(participantName)}
                  </div>
                )}
                ...
                <p className="mt-1 text-sm text-slate-500">
                  {currentThread?.listing?.title || "Hostel conversation"}
                </p>
                  ...
                  if (item.type === "divider") {
                    return (
                      <div key={item.key} className="flex justify-center py-2">
                        <span className="rounded-full bg-white/90 px-3 py-1 text-xs font-medium text-slate-400 shadow-sm ring-1 ring-slate-200">
                          {item.label}
                        </span>
                      </div>
                    );
                          ...
                          {hasAttachment(message) ? (
                            ...
                            >
                              <span className="truncate font-medium">
                                {message.attachmentName || "Agreement PDF"}
                              </span>
                              <span
          ...
          {showScrollToBottom ? (
            <button
              ...
              className="absolute bottom-24 left-1/2 z-10 inline-flex h-12 w-12 -translate-x-1/2 items-center justify-center rounded-full border border-slate-200 bg-white/95 text-slate-600 shadow-lg shadow-slate-200 transition-all duration-200 hover:bg-white hover:text-blue-600"
    ...
    if (currentListing && user?.role === "renter") {
      ...
      const ownerName = getParticipantName(owner);
      ...
      return (
        <div className="flex h-full min-h-0 w-full flex-col overflow-hidden bg-[#f4f7fb]">
          ...
          <div className="min-h-0 flex flex-1 items-center justify-center bg-[radial-gradient(circle_at_top,_rgba(59,130,246,0.08),_transparent_30%),linear-gradient(180deg,_#f6f9fd_0%,_#eff4fb_100%)] px-6 py-10">
            <div className="flex w-full flex-col items-center text-center">
              {ownerImage ? (
                ...
                <div className="flex h-24 w-24 items-center justify-center rounded-full bg-slate-200 text-xl font-semibold text-slate-700 shadow-sm">
                  {getInitials(ownerName)}
                </div>
              )}

              <h3 className="mt-6 text-3xl font-semibold text-slate-900">{ownerName}</h3>
              <p className="mt-2 text-base text-slate-500">
                {currentListing?.title || "Hostel listing"}
              </p>
              ...
              <p className="mt-6 max-w-lg text-sm leading-7 text-slate-500">
                Write your first message here. Ask about availability, rules, deposit, or
                anything else before sending your booking request.
              </p>
            </div>
          </div>
    ...
    return (
      <div className="hidden h-full items-center justify-center bg-[linear-gradient(180deg,_#f7faff_0%,_#eff5fb_100%)] lg:flex">
        <div className="max-w-md px-6 text-center">
          <div className="mx-auto flex h-20 w-20 items-center justify-center rounded-full bg-white text-slate-400 shadow-sm ring-1 ring-slate-200">
            <Building2 className="h-9 w-9" />
          </div>
          <h3 className="mt-5 text-2xl font-semibold text-slate-900">Choose a conversation</h3>
          <p className="mt-3 text-sm leading-7 text-slate-500">
            Open a thread from the left and continue the conversation in a Messenger-style inbox.
          </p>
        </div>
      </div>
    );
  ...
  const content = (
    <div className="flex h-full min-h-0 w-full flex-col">
      <div className="mb-5 flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-slate-900">Chats</h1>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          >
            {user?.role === "owner" ? "View inquiries" : "View list"}
          </button>
        </div>
      </div>

      <div className="min-h-0 flex-1 overflow-hidden rounded-[32px] border border-slate-200 bg-white shadow-[0_24px_80px_-36px_rgba(15,23,42,0.35)]">
        <div className="grid h-full min-h-0 grid-cols-1 lg:grid-cols-[370px_minmax(0,1fr)]">
          <aside
            className={`min-h-0 flex flex-col bg-[#f8fafd] ${
          ...
          >
            <div className="border-b border-slate-200 px-5 py-5">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <h2 className="text-3xl font-bold tracking-tight text-slate-900">Chats</h2>
                  <p className="mt-1 text-sm text-slate-500">
                    {threads.length} conversation{threads.length === 1 ? "" : "s"}
                  </p>
                </div>
                <div className="rounded-full bg-white px-3 py-1 text-xs font-semibold text-blue-600 shadow-sm ring-1 ring-slate-200">
                  Inbox
                </div>
              </div>

              <div className="mt-5 flex items-center gap-3 rounded-[22px] bg-white px-4 py-3 shadow-sm ring-1 ring-slate-200">
  ...
  return (
    <div className="h-screen overflow-hidden bg-gradient-to-br from-slate-100 via-white to-blue-50">
      <Navbar />
