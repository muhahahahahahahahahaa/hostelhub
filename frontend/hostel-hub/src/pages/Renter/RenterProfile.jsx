...
import { Bell, FileText, Save, Upload, X } from "lucide-react";
...
const RenterProfile = () => {
  ...
  useEffect(() => {
    ...
    const loadInquiries = async () => {
      try {
        ...
        const response = await axiosInstance.get(API_PATHS.INQUIRIES.GET_MINE);

        if (isMounted) {
          setInquiries(Array.isArray(response.data) ? response.data : []);
        }
      } catch (error) {
        console.error("Failed to load renter inquiries", error);
        if (isMounted) {
          setInquiries([]);
        }
      } finally {
  ...
  const confirmedNotifications = useMemo(
    () =>
      inquiries.filter(
        (inquiry) => inquiry?.status === "Confirmed" && !inquiry?.agreementAcceptedAt,
      ),
    [inquiries],
  );
                  ...
                  <div className="mt-4 space-y-3">
                    {loadingNotifications ? (
                      ...
                      confirmedNotifications.map((inquiry) => (
                        <div
                          key={inquiry._id}
                          className="flex flex-col gap-3 rounded-2xl border border-emerald-100 bg-white px-4 py-4 shadow-sm sm:flex-row sm:items-center sm:justify-between"
                        >
                          <div>
